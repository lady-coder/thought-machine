module "turn_on_lambda_log_group" {
  ...
  log_group_name         = lower("/aws/lambda/${aws_lambda_function.lambda_turn_on_spacelift_workers.function_name}")
  cloudwatch_kms_key_arn = var.kms_cloudwatch_arn
  retention_in_days      = var.retention_in_days
}

data "archive_file" "lambda_turn_on_spacelift_workers" {
  type        = "zip"
  source_file = "${path.module}/python/lambda-turn-on-off-spacelift-workers.py"
  output_path = "${path.module}/python/lambda-turn-on-spacelift-workers.zip"
}

resource "aws_lambda_function" "lambda_turn_on_spacelift_workers" {
  ...
  filename                       = "${path.module}/python/lambda-turn-on-spacelift-workers.zip"
  function_name                  = lower("${var.environment}-${var.component}-lambda-turn-on-spacelift-workers")
  role                           = aws_iam_role.role_turn_on_spacelift_workers.arn
