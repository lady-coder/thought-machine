module "turn_off_lambda_log_group" {
  ...
  log_group_name         = lower("/aws/lambda/${aws_lambda_function.lambda_turn_off_jumphost.function_name}")
  cloudwatch_kms_key_arn = var.kms_cloudwatch_arn
  retention_in_days      = var.retention_in_days
}

data "archive_file" "lambda_turn_off_jumphost" {
  type        = "zip"
  source_file = "${path.module}/python/lambda-turn-off-jumphost.py"
  output_path = "${path.module}/python/lambda-turn-off-jumphost.zip"
}

resource "aws_lambda_function" "lambda_turn_off_jumphost" {
  ...
  filename                       = "${path.module}/python/lambda-turn-off-jumphost.zip"
  function_name                  = lower("${local.name}-lambda-turn-off-jumphost")
  role                           = aws_iam_role.role_turn_off_jumphost.arn
  handler                        = "lambda-turn-off-jumphost.lambda_handler"
  runtime                        = "python3.8"
