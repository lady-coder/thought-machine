module "turn_off_dbs_lambda_log_group" {
  source  = "spacelift.io/gft-blx/cloudwatch-log-group/aws"
  version = "1.0.1"

  log_group_name         = lower("/aws/lambda/${aws_lambda_function.lambda_turn_off_dbs.function_name}")
  cloudwatch_kms_key_arn = var.cloudwatch_cmk_arn
  retention_in_days      = var.retention_in_days
}

data "archive_file" "lambda_turn_off_dbs" {
  type        = "zip"
  source_file = "${path.module}/python/lambda-turn-on-off-dbs.py"
  output_path = "${path.module}/python/lambda-turn-off-dbs.zip"
}

resource "aws_lambda_function" "lambda_turn_off_dbs" {
  #checkov:skip=CKV_AWS_116: "Ensure AWS Lambda function is configured for a DLQ"
  #checkov:skip=CKV_AWS_173: "Check encryption settings for Lambda environmental variable"
  #checkov:skip=CKV_AWS_272: "Ensure AWS Lambda function is configured to validate code-signing"
  filename                       = "${path.module}/python/lambda-turn-off-dbs.zip"
  function_name                  = lower("${var.environment}-${var.component}-lambda-turn-off-${var.cluster_identifier}")
