...
#module "dcr_service_alb_prerequisites" {
...
#  domain_names                    = ["dcrs.${var.public_domain_name}"]
...
module "dcr_service_irsa_role" {
  ...
  namespace        = "dcr-service"
  application_name = "dcr-service"
}

resource "aws_iam_role_policy_attachment" "dcr_service_appmesh_envoy" {
  role       = module.dcr_service_irsa_role.irsa_role_name
  policy_arn = "arn:aws:iam::aws:policy/AWSAppMeshEnvoyAccess"
}

data "aws_iam_policy_document" "dcr_service_aurora" {
  statement {
    sid       = "RdsConnect"
    actions   = ["rds-db:connect"]
    effect    = "Allow"
    resources = ["arn:aws:rds-db:${var.region}:${local.account_id}:dbuser:${var.apps_aurora_cluster_resource_id}/document_service_iam_user"]
  }
}

resource "aws_iam_role_policy" "dcr_service_aurora" {
  name   = "dcr-service-aurora"
  policy = data.aws_iam_policy_document.dcr_service_aurora.json
  role   = module.dcr_service_irsa_role.irsa_role_name
}
